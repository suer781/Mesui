...
允许**隐私群 → 效率群 单向转换**：管理员发起，在线成员即时确认并完成 MLS
加入；离线成员经信箱异步公告（附 mailbox secret 的 HMAC 证明，同 B1–B5 约束），
上线时由管理员替其完成 MLS Add+Commit。过渡期（尚有成员未入 MLS 树）群消息
**双写**：pairwise 逐成员单发与 MLS 整树加密并存；全员入树或管理员强制
finalize（超时未上线者移除出群）后结束。转换后选择性可见功能永久丧失；
**不允许反向转换**——技术上旧 pairwise 会话仍在、可重建逐成员加密，但反向
转换会让元数据重新暴露给中继、功能语义反复摇摆，故属产品/隐私策略层面的
永久禁止（实现不复原任何 pairwise 群状态）。「群管理员」为新增角色概念，
实现前需先定义角色与权限模型。

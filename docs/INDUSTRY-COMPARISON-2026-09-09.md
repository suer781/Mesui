...
| 最严重的缺点 | **P0×1：BLE 重连握手失效**——「33/32 幽灵」第三次出现，近距离好友重连主链路跑不通 |
...
**P0-1 BLE 重连握手失效——「33/32 幽灵」第三次现身**
- `BleMesh.kt:319-321`：AUTH_CHA body = nonce(16)+identityKey(33)=49B，但断言 48B 且只切 32B 比对，永不匹配 → 布隆命中后回连握手两端都建不起来。
- 说明：此前修复只覆盖了二维码路径（AddFriendPayload 33 字节），BLE 路径漏网。**同类缺陷三连：QR 路径 / contacts.rs / BleMesh AUTH**，根因是身份长度契约没有单一事实源。
- 修复：全仓统一 `ID_LEN=33` 常量 + 补「Kotlin 构造真实载荷 → Rust 解析」的跨 FFI 集成测试（这是审计报告遗留建议 #7）。
...
| P1-2 | contacts.rs upsert 强拒 33 字节身份 | contacts.rs:117-125 | 33/32 幽灵第二处：好友落库必失败（存疑：取决于 Kotlin 传参） |
...
1. **跨语言字节契约无单一事实源** → 幽灵 bug 三连。解法：`ID_LEN` 单点定义 + 跨 FFI 集成测试。
...
| 阶段 | 内容 | 验证 |
|---|---|---|
| Hotfix | P0-1 + P1-1/2（33/32 与 TOFU 收口）+ P1-4 首聊丢消息 + P1-5 静默清库 | 单测 + 真机两台重连 |
